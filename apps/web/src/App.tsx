...
function WorkspaceApp(): React.ReactElement {
 const { isAuthenticated, isLoadingAuth } = useWorkspace(); const [activeTab,setActiveTab]=useState<NavTab>("dashboard"); const [searchOpen,setSearchOpen]=useState(false);
 useEffect(()=>{const h=(e:KeyboardEvent)=>{if((e.ctrlKey||e.metaKey)&&e.key.toLowerCase()==="k"){e.preventDefault();setSearchOpen(true);}};window.addEventListener("keydown",h);return()=>window.removeEventListener("keydown",h);},[]);
 ...
 return <div className="app-container animate-fade-in"><Sidebar activeTab={activeTab} onTabChange={setActiveTab}/><div className="main-content"><Header activeTabTitle={tabTitles[activeTab]} onSearch={()=>setSearchOpen(true)}/>{activeTab==="dashboard"&&<DashboardView/>}{activeTab==="ai"&&<AIAssistant/>}{activeTab==="projects"&&<ProjectsView/>}{activeTab==="tasks"&&<TasksView/>}{activeTab==="crm"&&<CRMView/>}{activeTab==="system"&&<SystemView/>}</div><GlobalSearchModal open={searchOpen} onClose={()=>setSearchOpen(false)} onNavigate={tab=>setActiveTab(tab)}/></div>;
}
